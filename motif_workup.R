...
# Print plots as pdf
X11()
plot(-log10(t_total[order(t_total$T.L.adj),]$T.L.adj), type = "h", xlim = c(0,400), ylim = c(0,30), ylab = "Motif Enrichment", axes = FALSE, main = "Motif enrichment in differentially marked enhancers")
axis(1, cex.axis = 1.5)
axis(2, cex.axis = 1.5)
dev.copy(pdf, "motif_full.pdf", width=7, height=7)
dev.off()
plot(-log10(t_total[order(t_total$T.L.adj),]$T.L.adj), type = "h", xlim = c(0,50), ylim = c(0,30), ylab = "Motif Enrichment", axes = FALSE, main = "Motif enrichment in differentially marked enhancers")
axis(1, cex.axis = 1.5)
axis(2, cex.axis = 1.5)
